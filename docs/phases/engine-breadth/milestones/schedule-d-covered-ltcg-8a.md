...
Phase: Engine Breadth. Selected by the owner 2026-08-01; reconstructed from
the preserved pre-curation state on 2026-08-02.
...
- **Establish scope:** applies through the first commit of the draft milestone
  PR. This reconstructed plan incorporates the scope learned before the reset.
...
- **Build:** applies after the contract commit exists on this milestone branch.
...
The preserved prototype round discharged these gates before the reset. Its
material evidence is curated under
`docs/archive/2026-08-02-schedule-d-covered-ltcg-evidence/`; the complete
pre-curation state remains at
...
Each track charter names its focused module commands. Before the draft
milestone PR is updated, the Builder may run the full local gate once; CI
`verify` on the final candidate remains the gate of record.
...
branch range, coverage-frontier update, deferral dispositions, and data-safety
scan before the milestone PR is marked ready.
...
8. All production tracks pass independent review, and CI `verify` is green on
   the exact final milestone PR head.
...
### Contract evidence stage — not a production track

Curate the already-completed paper-first incumbent/rival evidence for P1 and
P2 and the paper spikes for P3 and the two later substrate gaps. Commit the
accepted ADRs and their bounded evidence archive before production code. Raw
working ceremony remains available only through the snapshot. No production
code belongs in this commit.
...
README capability summary where accurate, and retrospective. Remove working
charters and other unpromoted execution records. Obtain fresh review of the
curated branch range before the milestone PR is marked ready.

## Sequencing and economy

The contract evidence stage precedes sequential production Tracks 1 → 2 → 3,
followed by closeout. There is no parallel-work manifest because the tracks
share the accepted contract, published content/package surface, authoritative
goldens, and presentation fixture. Temporary role charters may be committed
while the draft PR is open and are removed or distilled before final review.
