...
| Current-year covered capital losses and Schedule D line 21 limitation | Returns whose only capital-transaction activity is one or more covered, basis-reported, no-adjustment Form 1099-B transactions — short-term or long-term, gain or loss — reported directly on Schedule D line 1a or 8a without Form 8949, with the current-year $3,000/$1,500 capital-loss limitation and no inbound carryover | No short-term source family; the existing long-term family's closure predicate is ratified gain-only by decision (ADR-0052 Decision 1), not merely by schema, and cannot be silently reinterpreted; `selected-preferential-base`'s discriminator cannot select Schedule D for a short-term-only return; no signed line 16/line 21/line 7a successor exists | Additive successor long-term family (gain-or-loss) and new short-term family, each preserving the original gain-only family unedited; a `selected-preferential-base` successor discriminated over both families' closure with a floored (nonnegative) preferential amount; signed Schedule D lines 1a/7/15/16 and line 21; an additive completeness successor retiring two of the seven boundary declarations while preserving the rest | Paper-first Track 0 settling the source/routing/completeness/pin contracts, then production-shaped synthetic coordinator, lifecycle, completeness, package, explanation, and presentation evidence, including every prior-milestone regression fixture | **selected** |
| Inbound capital-loss carryovers | Returns whose current-year Schedule D includes a capital-loss carryover from a prior tax year | No carryover source, authority, or year-boundary contract; deliberately excluded from the current-year-losses slice | A distinct prior-year-amount source, its own completeness authority, and 2026 carryforward derivation and publication (not attempted by the current-year-losses slice) | Its own paper-first contract evidence once selected | candidate |
