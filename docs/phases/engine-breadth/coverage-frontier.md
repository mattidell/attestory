...
| Covered, long-term, gain-only capital transactions → Schedule D line 8a | Returns whose only capital-gain source is one or more covered, long-term, gain-only, no-adjustment Form 1099-B transactions eligible for direct Schedule D line 8a reporting without Form 8949 | No transaction source family, Schedule D production content, or the completeness boundary against short-term/loss/carryover/8949/other-Schedule-D sources | New transaction-identity contract one level below statement identity, a nine-part completeness boundary via component authority, and a Schedule D instantiation of the accepted attachment ontology (ADR-0036) reused as content only; must not manufacture a schedule from box 2a alone and must not edit ADR-0050 in place | Paper-first prototype evidence (`docs/prototypes/schedule-d-covered-ltcg-8a/`), then production-shaped synthetic coordinator, lifecycle, completeness, package, explanation, and presentation evidence | **selected** |
