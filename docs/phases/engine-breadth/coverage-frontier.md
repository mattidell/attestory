...
| Noncovered / basis-not-reported Form 8949 transactions | Returns with noncovered securities, broker-basis corrections, or other Form 8949 adjustment codes besides W | Paused: the ratified engine hard-codes current Form 8949 row validation, identity constraints, and consumer routing in generic Python; the prerequisite declarative-validation milestone must merge first | After the prerequisite, this becomes content instantiation over the accepted generic structured-validation and consumer-dependency substrate; proposed ADR-0063/0064/0065 remain unratified and must be redesigned | Prerequisite Track 0 contract and implementation first; then a re-cut/rebased noncovered-basis milestone | **paused — prerequisite selected** |
