...
## Matrix (as of 2026-07-26, Presentation L2 Integration Grounding records)
...
5. **Presentation remains L2 after L2 Integration Grounding.** Presentation —
   ...
   | Browser path | Two synthetic manifests only; no live invocation vehicle | `tools/presentation_harness/examples/manifests/citation-walk.v1.json`; `tools/presentation_harness/examples/manifests/citation-walk-production-shaped.v1.json` |
   | Real operation | Not exercised; no owner attestation and no live browser invocation | Presentation remains L2 |
...
1. **Presentation L2 → L3 — available, not selected.** The projector and
   renderer are now production-shaped and synthetic end-to-end (footnote 5).
   What remains is not “just run it”: the repository has no data-boundary-safe
   live browser invocation vehicle, browser profile/cache containment, or
   owner-attested real operation. A future owner-selected milestone must first
   choose and verify that live boundary, then perform the non-descriptively
   attested real exercise. The synthetic repository harness is not that
   vehicle.
