...
**3. Does the viewing preflight cover an entry session?** ADR-0048 flagged
that an entry session creates content and the preflight was written for
sessions that only read. Everything here is synthetic, so nothing typed is
real, but the surface being built is the one that will later carry real typing.
Track 1 should note what it relies on rather than quietly extend anything.
