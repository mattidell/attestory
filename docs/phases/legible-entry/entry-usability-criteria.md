...
- **Dependencies for running the evaluation (not confirmed by Track 0):** Track 1 must establish that a synthetic workspace can be seeded with every required non-W-2 fact so W-2 is the only missing family; that the entry surface can be served at a URL and can send contributions through the admission path; and that the surface makes the fixed W-2 evaluation sets and the zero-missing, fully computed state observable. The evaluation does not run until all three dependencies are demonstrated. Naming them here does not confirm them.
