...
- First Tax Slice — **planned; evidence gate pending** (plan: `milestones/first-tax-slice.md`; planning input: `milestones/first-tax-slice-inputs.md`). Pure content on the finished machinery; authored after the cascade patch so it can carry a supersession acceptance golden. Implementation waits on Track 0's form-field/fact-type/source-closure/rendered-absence contract decision.
