...
→ discriminating evidence → production build → simple abstraction. A first
attempt that built the full slice in one pass was returned NOT READY by
independent review because several architectural decisions were fused into
one artifact, making it hard to attribute the failure to a specific choice.
The milestone restarted as a decomposition into six independently
chartered seams (canonical value extraction, identity association,
relationship constraints, standing authorization and currentness, rule-owned
consequences, and ordinary input mapping), each run with rival builders and a
clean-room/adversarial/eligibility review committee where a rival is
genuinely informative, followed by one integration checkpoint. Its
...
| Document and Ordinary-Fact Translation Vertical | **Restarted 2026-08-28 as six seam-decomposed prototypes + integration** | Fluid domain model, canonical workspace slice, structured ordinary input, tax derivation, line-2b production path |
