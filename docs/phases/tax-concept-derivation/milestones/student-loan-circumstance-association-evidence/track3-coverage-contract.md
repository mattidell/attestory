...
The production admission of the two names is `live._resolved_run_material`. That function builds `collect_names` and the live coordinator passes the list to marshal as `collect_source_names`. Today the list is source-family member predicates, their companions, names from `_iter_collect_categorical_names`, and four rule-id special cases. It does not read a `link_coverage` node. The function changes in two places. Its rule-schema set, which today stops at `rule-artifact.v9`, gains `rule-artifact.v10`, or a v10 rule is not in `rules` and the walk never sees it. For every rule in that list it walks `value`, by the same recursion `_iter_collect_categorical_names` uses, and for every `link_coverage` node it appends that node's `links` and `reductions` strings to `collect_names` when they are not already present. The walk adds no source-family member, no companion, and no closure read. It is not `_iter_collect_source_sets`, and `audit_collect_authority` does not see the node. `marshal_run_context` does not change. That same `collect_names` set is what the input-binding loop and the legacy fallback consult, and what fills the sources an ordinary `collect`, a closed `count`, or a `collect_categorical_all_equal` reads. Appending these two names would change any other member that bound, required, referenced, collected, or counted them. Section 2 confines those uses in `package_validation.validate_package`. It does not confine them by taking the names out of the set those branches consult, and it does not change `marshal_run_context`. Marshal already emits a `SourceFact` only for a name in `collect_source_names`, and only for a current kernel finding whose fact type matches (`currency.current_finding_ids`, then the fact-type test). A current link is admitted by that existing loop once its name is on the list. A reduction is a derived finding. Marshal never loads derived findings, so registering `reductions` does not emit one. The reduction becomes a source only when its rule appends a same-run live source (`runner._Run._append_live_source`, from `_append_live_source_from_finding`). A validated package cannot reach a run in which the link type was never collected: package validation requires both names, and `_resolved_run_material` registers both before marshal. When that collected type has no current finding, per-subject dispatch installs an empty slot (section 3). That empty slot is the default in section 5. It is not a missing admission, and it is not `link-coverage-scope-unbound`.
...
- The node is not walked by `_iter_collect_source_sets` and not seen by `audit_collect_authority`. It adds no source-family edge and no closure read. That refusal is not the collect-name admission. `live._resolved_run_material` registers `links` and `reductions` as collected names, as section 1.1 specifies. `marshal_run_context` is not given a new emission rule.
...
**Name confinement.** `validate_package` accepts a package that contains a `link_coverage` node only when the two strings that node names are used by no other member, except as follows. This is the check that keeps the shared `collect_names` write from changing anyone else. The function that changes is `package_validation.validate_package`. `marshal_run_context` does not change. `marshal._rule_required_symbols` does not change. `evaluator.evaluate` does not change. `live._resolved_run_material` still only gains the registration section 1.1 already specifies.

- `links` may be named by exactly: this coverage node, as its `links` field; and the member whose `publishes` equals the node's `reductions` (the reduction rule). That rule reads the link as its per-subject subject by a `ref` of `links`. That `ref` is allowed. The reduction rule may not also name `links` in `requires`, a `collect`, a `count`, or a `collect_categorical_all_equal`, and no input binding may name it.
- `reductions` may be named by exactly this coverage node, as its `reductions` field. It is the `publishes` of that predecessor. The `publishes` value is the allowed naming. It is not a `ref`. No member, including the predecessor, may name `reductions` in `requires`, an input binding's `symbol` or `fact_type.id`, a `ref`, a `collect`, a `count`, or a `collect_categorical_all_equal`.
- No other member may name either string in `requires`, an input binding's `symbol` or `fact_type.id`, a `ref`, a `collect`, a `count`, or a `collect_categorical_all_equal` — **nor in any symbol an attachment rule declares**, which `marshal._rule_required_symbols` reads before `requires` and `ref` for attachment schemas: `requirement.subtotals`, a trigger's `subtotals`, `completeness.required_answers[].symbol`, `branch_requirements[].when_answer.symbol`, and `adds_required[].symbol`. Generally: any string `_rule_required_symbols` returns for any member other than the reduction rule is a reuse. The node's own fields are not those ops. `_iter_ref_names` yields a string only when `op == "ref"`, and `_iter_collect_exprs` yields only `op == "collect"`, so neither treats the node's fields as a hit. `count` and `collect_categorical_all_equal` carry the string in `name`. No existing iterator yields those names, so the new check in `validate_package` walks them. Input bindings are package fields, not rule members; the check reads `package["input_bindings"]` in the same pass. A second `link_coverage` node that repeats either string is another member naming it, and it is rejected.

The finding code is `LINK_COVERAGE_NAME_REUSED`. One `MemberIssue` per offending use. When the use is on a rule, `member_id` and `version` are that rule's. When the use is an input binding, `member_id` is the package id and `version` is empty, the same shape as `BINDING_FACT_TYPE_NOT_ADMITTED`. The reduction rule's `ref` of `links`, and its `publishes` equal to `reductions`, do not raise it. The coverage rule's own `requires`, and a `ref` of either string outside the node, raise the same code. The node placed in `when` still fails the shape check already stated above. That failure is not given this code.

Why this confines every effect of the shared registration, without a marshal change:

- **Scalar binding.** The binding loop in `marshal_run_context` binds `matches[0]` when `len(matches) == 1 or symbol not in collect_names`. `symbol` is `binding["symbol"]`. `matches` are the current findings of `binding["fact_type"]["id"]`. Disagreeing multi-matches were already left unbound. The registration changes the agreeing case only: an input binding whose `symbol` is `links` or `reductions`, and which has several agreeing current findings of its fact type, used to bind `matches[0]` and would not. One finding on an explicit binding still binds, because `len(matches) == 1` does not consult `collect_names`. No input binding may use either string as `symbol`, so no binding takes that branch because of these two names. Forbidding `fact_type.id` as well is the other half. A binding whose fact type is `links` under some other symbol would still have bound `matches[0]`, because that other symbol is not a collected name. The ban means those findings are not that binding's match set. A sibling fact type still binds as today, including one finding and including several agreeing findings.
- **Legacy fallback.** The fallback skips a fact type id that is in `collect_names` before it asks whether any rule requires it, so it would no longer bind `links` as a scalar at all, including when exactly one current finding exists. `_rule_required_symbols` is `requires` plus, on v3 through v9 and on v10 once section 1.1 adds v10 to that frozenset, every `ref` name. The only permitted mention that puts `links` into that set is the reduction rule's `ref`. That rule does not read the legacy scalar. Per-subject dispatch installs the subject on `local_symbols[subject_type]`; the caller passes `subject_type`. The fallback scalar would have been one run-wide current finding under the type id, which is not that subject. No other member may `requires` or `ref` either string, so no other member was receiving the scalar and none loses it. `reductions` is not a kernel fact type. The fallback keys on `_fact_type_id`. It would bind a type whose id equals that string only when some rule requires the id. None may, and no input binding may name it, so the exclusion removes a scalar no permitted member reads.
- **Sibling `collect`.** `evaluator.evaluate`'s `collect` arm reads `env.sources`, which `_Run.__init__` copies from the sources marshal filled out of `collect_names`. A `collect` of `links` that today has no rows takes the empty path and blocks `SOURCE_SET_UNCLOSED` unless its `source_set` is closed. After the name is collected, current findings are rows, the empty path is not taken, and the arm returns each value as a decimal. A non-numeric value, including a JSON object, blocks `DEPENDENCY_INVALID` from `_as_decimal` (`not a number`), not a separate `VALUE_INVALID` code. No other member may `collect` either string, and the operation does not use the `collect` arm, so that path is not reachable in a validated package. The arm is unchanged.
- **Closed `count`.** `count` still blocks when `source_set` is not in `env.closed_sets`, and when the set is already closed it returns `len(rows)`. Those rows would include the newly marshalled link findings. No other member may `count` either string, so no closed count counts them. The arm is unchanged. `collect_categorical_all_equal` reads the same `env.sources` rows (and blocks `DEPENDENCY_ABSENT` when there are none). The same ban keeps it off both names.
...
The checks run in the order of that table. A keys-unavailable sentinel is reported before an orphan, and it is not reported as an empty link list. An orphan reduction is reported before an uncovered link on the same subject, so a stale reduction cannot be ignored in favour of a block that names only the link, and cannot be ignored in favour of the default.

`subject_dispatch.evaluate_subject_scoped_rule` installs the operation's slot for both names the `link_coverage` node declares, on every subject. It learns those names by walking `value`. It does not learn them by listing names that happen to occur in `sources`. For each declared name it takes the live sources of that name, which may be none, and calls `_scope`. `_scope` of an empty candidate list returns `[]` without reading keys. The slot receives that list. Empty and unbound are different. Empty links with no orphan reduction is the default path. That is the path when no link joined, including when no link exists anywhere in the run. It is not `link-coverage-scope-unbound`.

`link-coverage-scope-unbound` is only the slot left at its default. That is ordinary `attempt()`, whose `Environment` never installs the field. The per-subject function does not leave either declared name out. A validated package also cannot take the path where the link type was never collected: section 1.1 registers both names before marshal, so a current link finding of that type is a source. `_scope` itself does not change.
...
Marshal emits a `SourceFact` for a kernel finding that `currency.current_finding_ids` lists and whose fact type is a collected name (`marshal_run_context`). A link finding that currency does not list is not a source and is not a recorded link. A current link whose type was never collected is also not a source; section 1.1 puts that type on the collect list for a validated package, so that omission is not a production path. Derived findings are not in that loop. A reduction becomes a source only as a same-run live source, as section 1.1 says.
...
The link list for this subject is empty, and the orphan check did not fire. The empty list is the slot per-subject dispatch installed for `links`, including when that name has no source in the run. It is not the unbound slot. The node returns the parameter's `values` as a decimal. It does not return `[]`. It does not record a closure read.
...
- The `collect` and `count` arms in `evaluator.evaluate`, including empty-collect `SOURCE_SET_UNCLOSED` and count's requirement that `source_set` be in `closed_sets`. `collect_categorical_all_equal` is likewise unedited. Section 2 rejects a package in which any other member applies those ops, or a scalar binding, to `links` or `reductions`.
- `marshal_run_context`, including the input-binding loop and the legacy fallback, and `marshal._rule_required_symbols`. A sibling fact type's scalar binding stays on today's branch because its symbol is not one of the two collected names.
...
17. **Confined names.** Validation rejects a sibling member for each forbidden use of `links` and of `reductions`: `requires`; an input binding's `symbol`; an input binding's `fact_type.id`; a `ref`; a `collect`; a `count`; a `collect_categorical_all_equal`; and an attachment rule naming either string in `requirement.subtotals`, a trigger's `subtotals`, or a completeness answer or branch symbol. Each rejection is `LINK_COVERAGE_NAME_REUSED`. The reduction rule's `ref` of `links`, and its `publishes` equal to `reductions`, are accepted. A package that obeys the rule marshals every other rule's scalar inputs as it did without these two names on `collect_names`. The witness is a sibling fact type: one current finding still binds, and several agreeing current findings still bind `matches[0]`. The link rows are present as sources. That is the registration, not a change to the sibling's binding. The sibling does not lose a legacy fallback scalar of either string. No `collect` of either string is accepted, so none returns those rows or blocks `DEPENDENCY_INVALID` from `_as_decimal`. No `count` of either string is accepted, so no closed count counts the new rows.
...
| Axis | Score | Reason |
| --- | --- | --- |
| Future blast radius | 2 | The operation is general grammar, not one rule id. Admitting it touches every rule-schema admission set, and `live._resolved_run_material` must register both declared names as collect names. `CURRENT_RECORD_SCHEMA` does not move: an uncovered link reuses `DEPENDENCY_INVALID` on `derivation-record.v9`. The `collect` and `count` arms, closure, currency, existing rule bytes, and the record schema do not move. `marshal_run_context` does not move. Another member's scalar binding, legacy fallback, `collect`, or closed `count` does not move either: `validate_package` rejects those uses of the two names (section 2). |
