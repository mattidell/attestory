...
| D8 | The dispatcher follows a **corrected** target rather than a stale one | A2 | `run` | `tests/test_nominee_consequences_live.py::test_c5a_report_correction_yields_corrected_remainder` — a pairing consumer, both findings in the act log, the current one pinned and the stale one excluded. Ceiling: follows the current finding at the same `fact_id`; settles nothing about D9 |
...
| D13b | The production worksheet's universal fold is **not** isolated — one statement's "no" blocks the whole route | A3 | `run` | `tests/test_f1098e_student_loan_interest_agi_track6.py::TestPathJMultiStatementDisagreement` (both orderings) and `tests/test_sli_worksheet_line21_track3.py`. This is the incumbent behaviour, and it is the opposite of D13a |
| D16 | A telling can be published in the same run as amounts, without being what changes them | A3 | `run` | `tests/test_capital_gain_distributions_line7a_t2_coordinator.py::test_line7a_publishes_and_line9_includes_once` — the categorical conclusion and three amount symbols all publish in one run. Whether a telling changes an amount is decided by whether the amount rule *declares a dependency* on it, which is consumer wiring rather than a capability limit |
...
D1, D2, D3, D4, D5, D6, D8, D13a and D16 — each within its ceiling. In ordinary
...
publish that conclusion alongside amounts without being what changes them.
...
D7, D9, D10, D11, D12 and D14. In particular, a shape that depends on **an
