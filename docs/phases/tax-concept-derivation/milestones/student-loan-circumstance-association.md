...
([`track5a-review.md`](student-loan-circumstance-association-evidence/track5a-review.md)). **5b is complete** — both schedulers dispatch a declared-subject rule per subject and wait on predecessor resolution; a rule declaring `joined`/`direction` gets the runtime presence check; undeclared rules are unchanged. Two fail-open defects found in foreman review were fixed; independent review passed with no defects ([`track5b-review.md`](student-loan-circumstance-association-evidence/track5b-review.md)). **Correction (owner's review at `6ca54a8d`, 2026-09-24): Track 5 is not complete.** 5a and 5b establish **working dispatcher tests** — the dispatcher behaves as ADR 0076 says when a test supplies its sources and fact types — **not a working live declaration-to-source path.** An independent review reproduced three defects on the live path: standalone fact types and declared subject/joined types never reach marshalling (a validated subject-only rule with a current finding produced nothing); runtime identity ignores the pinned fact-type version; a run-wide scalar bypasses a declared relationship. **Track 5c** repairs them: [`charter-track5c-declaration-to-execution.md`](student-loan-circumstance-association-evidence/charter-track5c-declaration-to-execution.md). Reader implementation waits on it. No production content declares a subject yet; Part 3 remains open.

**Track 5c status (2026-09-25).** Round 1 (`61a29876`) and round 2 repaired the three live-path defects and a fourth of the same class (`optional_default` resolved by id alone), each reproduced first through `validate_package` → `_resolved_run_material` → marshalling → both runners; two independent reviews passed with notes ([`track5c-review.md`](student-loan-circumstance-association-evidence/track5c-review.md)). **Established:** a declared subject or joined type backed by standalone fact types reaches both schedulers as keyed sources; presence and `optional_default` read the exact pinned version; a declared joined type cannot be satisfied by a run-wide scalar. **Not established:** exact-version resolution for categorical domains (evaluator) and for field-ref / yes-no validation; a declared-subject rule with no subject rows records nothing. Both await the owner.

**Track 5d (owner, 2026-09-25):** the version a rule or binding names is the version whose definition and value are used, and declaration order changes neither the answer nor its provenance — categorical domains and symbol types, field-ref and yes/no validation, and default parameters through their whole chain. Evaluator changes authorized; zero-subject rules get tests, not a new disposition. Two contract choices (derived categorical typing; unversioned `parameter` references) are characterised and brought to the owner, not implemented. Charter: [`charter-track5d-exact-version-authority.md`](student-loan-circumstance-association-evidence/charter-track5d-exact-version-authority.md). Dispatch is grok only.

**Track 5d is complete (2026-09-25).** For default parameters, categorical domains and symbol types, field-ref and yes/no validation, tables and the attachment threshold, the version a rule or binding names is the version whose value is used and whose pin is recorded, in either declaration order; an absent exact version blocks. Independent review passed with notes and a confirming review of round 3 confirmed ([`track5d-review.md`](student-loan-circumstance-association-evidence/track5d-review.md)). **Awaiting the owner:** the two contract choices (derived categorical typing; unversioned `parameter` references) and the reported out-of-scope notes.
