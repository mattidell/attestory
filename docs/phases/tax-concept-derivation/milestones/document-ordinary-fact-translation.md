<!-- foreman-context-v1
{
  ...
  "status": "Restarted 2026-08-28 as a seam-decomposed prototype milestone. The prior single-track candidate on milestone/document-ordinary-fact-translation combined several architectural decisions in one build and was returned NOT READY by independent adversarial review; that branch is retained as reference evidence, not as a base. This branch decomposes the same objective into six discriminating seams plus one integration checkpoint, each run under the Prototype Economic Gates with rival builders where a rival is genuinely informative.",
...
## Why this milestone restarted

`milestone/document-ordinary-fact-translation` built the full canonical slice
in one pass (Track 0 domain mapping, Track 2 production translation) and an
independent adversarial rereview returned **NOT READY** even after repair.
Comparing two complete architectures — the incumbent or a full rival — would
make it difficult to know which specific choice caused a success or a
failure, and would be a costly combinatorial exercise on top of that.
