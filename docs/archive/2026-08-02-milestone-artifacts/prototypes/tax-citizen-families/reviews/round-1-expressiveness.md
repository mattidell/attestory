...
before opening `docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/examination-it1.md`.
...
```sh
find /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1 -path '*form-field*.json' -type f | sed 's#/tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/##' | sort
```
...
```sh
jq -r '.derived_findings[] | select(.symbol|test("form1040.line(1a|2b|9|11|12|15|16)")) | [.id, .symbol, ((.pins // [])|join(","))] | @tsv' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/scenarios/synthetic-slice.json
```
...
```sh
jq -r '.sources[] | select(.id|test("form1040.line(1a|2b|9|11|12|15|16)")) | [.id, .locator] | @tsv' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/source-catalog.json
```
...
```sh
rm -rf /tmp/tcf-it1-review && mkdir -p /tmp/tcf-it1-review && git archive 88f0139 docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1 | tar -x -C /tmp/tcf-it1-review && python3 /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/validators/validate_it1.py /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1
```
...
root = Path('/tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1')
...
```sh
jq -r '.source_findings[] | [.id, .fact_id, .symbol, ((.identity_keys|keys_unsorted? // [])|join(",")), ((.evidence_ids // [])|join(","))] | @tsv' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/scenarios/synthetic-slice.json
```
...
```sh
jq -r '.id + "\t" + .logical_symbol + "\t" + .printed_locator' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/examples/positive/form-field.form1040.line2b.json
```
...
```sh
jq -r '.rendered_absence_states[] | [.state, (.publish_finding|tostring), (.artifact_guard // "none"), .explanation_terminal] | @tsv' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/scenarios/synthetic-slice.json
```
...
```sh
jq -r '.authoritative_state as $auth | .source_sets[] | [.source_set, .closure_state, (.closure_finding_id // "null"), (.gap_code // "null"), ($auth|tostring)] | @tsv' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/examples/positive/coverage-report.open-interest.json
```
...
```sh
jq -r '.mutations.coverage_rebuild | [.deleted_projection, .byte_equal, .stale_injected_projection.closure_state, (.stale_injected_projection.closure_finding_id // "null"), .stale_result, .reason] | @tsv' /tmp/tcf-it1-review/docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/it1/scenarios/synthetic-slice.json
```
...
`git diff --check -- docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/reviews/round-1-expressiveness.md`
produced no output. Because the file is untracked, I also ran
`git diff --check --no-index /dev/null docs/archive/2026-08-02-milestone-artifacts/prototypes/tax-citizen-families/reviews/round-1-expressiveness.md`;
