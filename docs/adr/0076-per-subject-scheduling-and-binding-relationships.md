...
- Status: **Parts 1 and 2 accepted** (owner, 2026-09-24) as the bounded scheduling contract and the
  relationship-validation contract. **Part 3 is open**: no option is selected, and nothing here authorizes
  publishing a per-statement amount through the link-to-status edge.
...
- Date: 2026-09-24
- Read at: branch `milestone/student-loan-circumstance-association`, HEAD `f0f1dd7630c330f5f1c6c840b8270152641eb8d2`. The probe write-up records itself at `b558da4e`. The draft did not re-run the probes.
- **Acceptance conditions (owner, 2026-09-24).** Behaviour tests for Parts 1 and 2 exercise rules that carry the
  new declarations. A rule without them (v1–v10) keeps today's behaviour, so the hand-dispatched probes that record
  today's joins — including the rows dropped by `_scope` — are not flipped: they remain evidence of undeclared
  behaviour. Implementation and independent review follow the publication plan below.
...
The workings, the full quotations, and the scoring are in [`adr0076-notes.md`](../phases/tax-concept-derivation/milestones/student-loan-circumstance-association-evidence/adr0076-notes.md). The operative text, quoted from the sources named there:
...
- Notes and quotations: [`adr0076-notes.md`](../phases/tax-concept-derivation/milestones/student-loan-circumstance-association-evidence/adr0076-notes.md)
