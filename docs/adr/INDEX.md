...
| 0056 | proposed | presentation, attachments | Not authority. Paper-spike proposal to close a Track-3-flagged gap: `_resolve_attachment` only ever produces a model entry for `published`; `blocked`/`inapplicable` attachments render nothing at all. Proposes a new top-level `attachments` model key mirroring `_resolve_field_row`'s disposition-tagged shape, leaving `citationGroups` byte-unchanged; declines extending `citationGroups` itself as more disruptive to existing goldens. Pending owner ratification. |
