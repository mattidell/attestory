...
| 0053 | proposed | attachments, packages | Discharges ADR-0052's two named production conditions (**not ratified**): `attachment-rule.v3` additive successor adding a categorical `family_nonempty` requirement trigger alongside the existing threshold shape; the selected-preferential-base symbol needs no new producer-selection substrate — one rule citizen with an internal `choose` branch, the existing single-rule pattern already used by line 7a/line 16, suffices and keeps the single-producer-per-symbol invariant intact. |
