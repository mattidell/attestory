...
| 0055 | proposed | attachments | Not authority. Paper-spike proposal to close a Track-2-flagged gap: ADR-0036's completeness check is presence-only, so an attachment can read `required-and-complete` while a value-checking consumer (e.g. `selected-preferential-base`) correctly blocks on the same facts. Proposes an additive `attachment-rule.v4` `check: "value"` shape and one bounded `attempt_attachment` branch; declines ADR-0038 Decision 5's admission-locus interlock as a category mismatch (eligibility conclusion, not factual contradiction). Pending owner ratification. |
