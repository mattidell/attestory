...
Most recent completed milestone: **Presentation — Real Session and Attestation**
— complete 2026-07-27. **The only milestone to date that moved the Presentation
row: L2 → L3 in the W-2 wages (1a) column**, on a real viewing session against
the owner's real residency plus the ADR-0031 Decision 7 non-descriptive
attestation with all five ADR-0047 preconditions affirmed individually. Three
tracks, one branch, one PR at close. Evidential basis: maturity-matrix
footnote 14. No next milestone is selected.
Plan:
`docs/phases/real-return/milestones/presentation-real-session-attestation.md`.
...
Real Session and Attestation is closed; nothing is chartered. The foreman's job
is to read `docs/phases/real-return/maturity-matrix.md` and present candidates.
Selection is owner-held.

**The cheapest live frontier is now the four remaining Presentation columns**
(Interest, Dividends, Return-level conditions, Schedule attachments). Nothing
needs building for them — the path is a single act that a human has now run
against both a synthetic workspace and the real residency. What is missing is
**record, not capability**: one attestation raises only what it covers, and the
owner named W-2 wages (1a) at plan stage. Extending the claim means an
attestation that names the other columns, which is not describing content.

Two things to carry into that rather than rediscover: the
**classified-refusal path has never been confirmed by a human** (a browser that
fails to start arriving as a stable reason code rather than a traceback rests on
tests and review only), and the **runbook has an unidentified unclarity** —
whoever uses it next should note where it reads badly at the moment it reads
badly.
