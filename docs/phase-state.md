...
- **Active milestone:** Nominee Interest Ownership Translation — **PLANNED;
  gates P1 and P2 COMPLETE and absorbed, P3 remains.** It is the first
  normalization test of the established translation cadence, using a bounded
  ownership/allocation case. P1 settled framing and product value; P2 settled the
  tax and artifact evidence, establishing the Schedule B belonging condition for
  the reduction but **no normalized information-reporting predicate** — literal
  IRC § 6049(a)(2), the IRS filing instructions, and 26 CFR 1.6049-4 retain
  separate authority-indexed formulations. P3 (experiments and track
  decomposition) is next and must test separation without settling the legal
  interpretation. Track 0 then proceeds through three reviewed checkpoints
  before any production unit. See
