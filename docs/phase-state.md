...
**➡️ Next: owner reviews and approves (or amends) the D2 (contribution) prototype plan** (`docs/prototypes/real-data-residency` sibling: `docs/prototypes/contribution-boundary/plan.md`), proposed on `plan/d2-contribution` alongside this pointer advance and the ADR-0031 accepted-flip. D2 is Tier 3 and interlocks with D1 (contribution writes real values *into* the ratified residency boundary). On plan approval (merge), Track 0's D2 topic opens with builder charters; **D3 (production resolver)** follows. Interim: the GitHub remote stays **private** (standalone owner decision to change).
