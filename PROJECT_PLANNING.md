...
- **Per-question progression.** Iteration scope and evidence depth are separate:
  ...
  iteration.
- **Gate 4 — Fixed caps and session-bounded cost review.** There is no cost-ratio trigger; session usage is already bounded, so a session boundary is the natural point to review the shape of cost incurred and still to come. Fixed caps apply and each forces stop-and-decide when crossed, never automatic charter expansion: two builder iterations including one rival; one owner-authorized repair pass; two default reviewers, a third only for a named uncertainty; context-starved legibility only when recoverability is itself a decision; and declared caps on build-artifact/check growth. **Review documents have no line cap**: their cost is bounded by the pre-declared measurement charter, and reviewers stop when those checks are completely reported rather than spending effort compressing valid evidence. Rival iterations are clean-room. A repair pass is named `repair<N>`, not as another iteration, and defaults to the original incumbent builder for deliberate defect continuity unless the disposition explicitly assigns a different builder.
