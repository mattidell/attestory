...
class TestSsaBoundaryUnit(unittest.TestCase):
    def test_box5_reconciliation_and_guards(self) -> None:
        ok = SsaBenefitsStatement(
            payer_id="demo.ssa.payer",
            tax_year=2025,
            statement_ref="claim-1",
            subject="taxpayer",
            statement_kind="ssa-1099",
            box3=1000,
            box4=200,
            box5=800,
            box6=0,
            lump_sum_election=False,
        )
        validate_statement(ok)
    ...
    def test_closed_empty_subtotal(self) -> None:
        pub = publish_subtotal(
            [],
            ...
            current_horizon_id="h0",
        )
        self.assertEqual(pub.value, Decimal(0))
        with self.assertRaises(SsaBenefitsError):
            publish_subtotal([], closure=None)
...
class TestSsaVerificationRepair(unittest.TestCase):
    ...
    def test_source_lifecycle_unit_boundary(self) -> None:
        def stmt(
            ref: str,
            box5: float,
            *,
            box3: float | None = None,
            box4: float = 0,
            ...
            b3 = box5 + box4 if box3 is None else box3
            return SsaBenefitsStatement(
                ...
                box5=box5,
                box6=box6,
                lump_sum_election=lump,
                corrected=corrected,
            )

        # Multiple statements aggregate.
        pub = publish_subtotal(
            [stmt("a", 1000), stmt("b", 2500)],
            closure=None,
        )
        self.assertEqual(pub.value, Decimal(3500))
        self.assertEqual(pub.statement_refs, ("a", "b"))

        # Same-identity correction supersedes.
        pub2 = publish_subtotal(
            [stmt("a", 1000), stmt("a", 1500, corrected=True)],
            closure=None,
        )
        self.assertEqual(pub2.value, Decimal(1500))

        # Duplicate rejection.
        with self.assertRaises(SsaBenefitsError):
            publish_subtotal([stmt("a", 1000), stmt("a", 1000)], closure=None)

        # Zero benefits with present member.
        pub0 = publish_subtotal([stmt("a", 0)], closure=None)
        self.assertEqual(pub0.value, Decimal(0))

        # Repayment equal to benefits → box5 zero.
        pub_eq = publish_subtotal(
            [stmt("a", 0, box3=500, box4=500)],
            closure=None,
        )
        self.assertEqual(pub_eq.value, Decimal(0))

        # Repayment exceeding benefits rejected (box5 != box3-box4 or negative).
        with self.assertRaises(SsaBenefitsError):
            publish_subtotal(
                [stmt("a", 0, box3=500, box4=600)],
                closure=None,
            )

        # Closed-empty zero pins closure; late horizon without re-closure is stale.
        closure = FamilyClosure(
            family_id=FAMILY, family_version="v1", horizon_id="h0", attested=True
        )
        empty = publish_subtotal([], closure=closure, current_horizon_id="h0")
        self.assertEqual(empty.value, Decimal(0))
        with self.assertRaises(SsaBenefitsError):
            publish_subtotal([], closure=closure, current_horizon_id="h1")

        # Guards: lump-sum, RRB, another taxpayer, nonzero withholding.
        with self.assertRaises(SsaBenefitsError):
            validate_statement(stmt("x", 100, lump=True))
        with self.assertRaises(SsaBenefitsError):
            validate_statement(stmt("x", 100, kind="rrb-1099"))
        with self.assertRaises(SsaBenefitsError):
            validate_statement(stmt("x", 100, subject="dependent"))
        with self.assertRaises(SsaBenefitsError):
            validate_statement(stmt("x", 100, box6=50))


    def test_live_late_member_stale_closure_and_reclosure(self) -> None:
        """Late member after closed nonempty family: aggregate updates; re-close restores authority."""
        # Start with one closed statement.
        acts = _ssa_acts(
        ...
        # Drop trailing adoption so we can append lifecycle acts, then re-adopt.
        adoption = acts.pop()
        # Current closed horizon is demo.ssa.h0 with one member.
        # Advance horizon with a late second statement without re-closing first.
        ...
        # Companions required before/with box5 — assert companions then the
        # transition already asserted box5. Need companions for admission.
        # The member-transition may fail companion presence; assert companions first.
        # Rebuild: companions before member-transition.
        # (If transition already added, insert companions before it.)
        # Simpler path: use plain assertions after horizon genesis on new horizon
        # without member-transition API if companions are hard.

        # Remove the incomplete transition and do full companion+box5 assertions
        # with horizon succession only for membership tracking.
        acts = [a for a in acts if not (
            a.get("kind") == "member-transition"
            and "late" in json.dumps(a)
        )]
        # Re-add entity if stripped
        if not any(
            cast(dict[str, Any], a.get("payload", {})).get("entity", {}).get("id") == statement
            for a in acts if a.get("kind") == "entity-introduced"
        ):
            acts.append(
                _act(
                    len(acts),
        ...
        # Horizon succession to record late membership for closure freshness.
        acts.append(
            _act(
                len(acts),
                "member-transition",
                {
                    ...
                    "member": {
                        "action": "assert",
                        "finding": _attested(
                            "demo.ssa.finding.box3.late",
                            f"{BOX3}|statement={statement},tax-year=2025",
                            4000.0,
                        ),
                    },
                    "successor": {
        ...
        # Present-source aggregate includes late member even though h0 closure is stale.
        adoption["committed_against"] = len(acts)
        acts.append(adoption)
        acts = _renumber(acts)
        report, model = _run(acts, run_id="demo.ssa.late.open")
        self.assertEqual(float(_published_numeric(model, "line-6a")), 12000.0)

        # Re-close on successor horizon — still publishes the same aggregate.
        acts2 = [a for a in acts if a.get("kind") != "package-adoption"]
        acts2.append(
            _act(
                len(acts2),
                "assertion",
        ...
        adoption2 = _load_fixture_adoption()
        adoption2["committed_against"] = len(acts2)
        acts2.append(adoption2)
        acts2 = _renumber(acts2)
        _, model2 = _run(acts2, run_id="demo.ssa.late.reclosed")
        self.assertEqual(float(_published_numeric(model2, "line-6a")), 12000.0)
        ordinary = self._ordinary_from_model(model2)
        expected = _expected_taxable_ss(12000.0, ordinary, 0.0)
        self.assertEqual(float(_published_numeric(model2, "line-6b")), expected)

    def test_admission_rejects_out_of_class_statements(self) -> None:
        """RRB/foreign, lump-sum, another-taxpayer, excess-repayment hard reject."""
        with self.assertRaises(SsaBenefitsError):
            validate_statement(
                SsaBenefitsStatement(
                    payer_id="p", tax_year=2025, statement_ref="r",
                    subject="taxpayer", statement_kind="rrb-1099",
                    box3=100, box4=0, box5=100, box6=0, lump_sum_election=False,
                )
            )
        with self.assertRaises(SsaBenefitsError):
            validate_statement(
                SsaBenefitsStatement(
                    payer_id="p", tax_year=2025, statement_ref="r",
                    subject="taxpayer", statement_kind="ssa-1099",
                    box3=100, box4=0, box5=100, box6=0, lump_sum_election=True,
                )
            )
        with self.assertRaises(SsaBenefitsError):
            validate_statement(
                SsaBenefitsStatement(
                    payer_id="p", tax_year=2025, statement_ref="r",
                    subject="dependent", statement_kind="ssa-1099",
                    box3=100, box4=0, box5=100, box6=0, lump_sum_election=False,
                )
            )
        with self.assertRaises(SsaBenefitsError):
            validate_statement(
                SsaBenefitsStatement(
                    payer_id="p", tax_year=2025, statement_ref="r",
                    subject="taxpayer", statement_kind="ssa-1099",
                    box3=100, box4=150, box5=0, box6=0, lump_sum_election=False,
                )
            )
