...
- **Prototype-process dispatch.** If the active milestone (per `docs/phase-state.md`) is a prototype-process milestone, do not start working generally: read the seat file (`docs/prototypes/<topic>/SEAT.md`) and take the seat it assigns, under that seat's role charter. Do not self-assign the foreman seat unless the seat file marks it vacant; record any succession in the process log. Committee reviewer seats — including prototype legibility, which is a normal reviewer, not a context-starved seat — are spawned as sub-agents by the foreman by default when it has that capability (ADR-0013), each at the plan's stage-appropriate tier. Starved fresh-reader rigor is not run per iteration; it lives in the periodic owner-spawned Legibility Audit (`docs/legibility-audits/`). When the foreman lacks sub-agent capability, reviewer seats are launched by the owner with the role file's launch line, and are never taken by generic resumption.
